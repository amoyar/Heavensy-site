...
async function handleLogin(e) {
  ...
  try {
    ...
    const companies = user?.all_companies || [];

    // Guardar empresas del usuario para uso en el panel
    localStorage.setItem('hs_user_companies', JSON.stringify(companies));
    localStorage.setItem('hs_user_roles', JSON.stringify(user?.roles || []));

    if (companies.length > 1) {
      // Múltiples empresas → guardar token temporal y mostrar selector
      _tempToken = access_token;
      _showCompanySelector(companies, user);
    } else {
