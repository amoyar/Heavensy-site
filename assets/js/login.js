...
// Detecta si estamos en pages/ o en la raíz para saber la ruta al panel
function _loginPanelUrl() {
    return window.location.pathname.includes('/pages/') ? '../index.html#dashboard' : 'index.html#dashboard';
}
...
document.getElementById('loginForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (res.ok && data.access_token) {
            ...
            setTimeout(() => { window.location.href = _loginPanelUrl(); }, 600);
        } else {
...
if (localStorage.getItem('token')) {
    window.location.href = _loginPanelUrl();
}
